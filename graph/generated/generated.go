...
func (ec *executionContext) _Query_noteById(ctx context.Context, field graphql.CollectedField) (ret graphql.Marshaler) {
	...
	res := resTmp.(*model.Note)
	fc.Result = res
	return ec.marshalONote2ᚖentᚑgraphqlᚋgraphᚋmodelᚐNote(ctx, field.Selections, res)
}
...
func (ec *executionContext) marshalNNote2ᚖentᚑgraphqlᚋgraphᚋmodelᚐNote(ctx context.Context, sel ast.SelectionSet, v *model.Note) graphql.Marshaler {
	if v == nil {
		if !graphql.HasFieldError(ctx, graphql.GetFieldContext(ctx)) {
			ec.Errorf(ctx, "must not be null")
		}
